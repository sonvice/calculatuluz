<template>
    <div class="d-grid">
      ...
      <div>
        ...
        <div class="d-flex">
          <input type="number" v-model="power" placeholder="Ej: 1500" min="0" max="3500"
            step="50" />
      ...
      <div>
        ...
        <input type="number" v-model="hours" placeholder="Ej: 2.5" min="0.5" max="24" step="0.5" />
        <small class="text-size--2">Usa punto decimal (ej: 1.5)</small>
      </div>
  
      <div class="button-group d-flex">
        <button class="btn" data-type="accent" @click="handleCalculate">Calcular consumo</button>
  ...
  // Computed properties
  const isCustomAppliance = computed(() => selectedAppliance.value === 'custom');
  const selectedApplianceLabel = computed(() => {
    const appliance = appliances.find(a => a.value === selectedAppliance.value);
